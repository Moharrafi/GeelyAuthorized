  ...
  return (
    <section className="relative min-h-screen bg-white dark:bg-slate-950 flex flex-col justify-center overflow-hidden py-32 px-6 transition-colors duration-1000 select-none">
      ...
      <div className="absolute inset-0 pointer-events-none transition-all duration-1000 ease-in-out">
        <div 
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full opacity-[0.55] dark:opacity-[0.75] blur-[140px] transition-all duration-1000 scale-110"
          style={{ background: `radial-gradient(circle, ${activeColor.hex}, transparent 65%)` }}
        />
        <div 
          className="absolute -top-[15%] -left-[10%] w-full h-full opacity-[0.35] dark:opacity-[0.5] blur-[150px] transition-all duration-1000"
          style={{ background: `radial-gradient(circle at top left, ${activeColor.hex}, transparent 60%)` }}
        />
        <div 
          className="absolute bottom-[-5%] left-1/2 -translate-x-1/2 w-[150%] h-[40%] opacity-[0.65] dark:opacity-[0.85] blur-[110px] transition-all duration-1000"
               ...
               <div className="grid grid-cols-1 gap-4">
                  {CAR_MODELS.map((model) => (
                    <button
                      ...
                      className={`relative w-full px-10 py-6 rounded-full text-left transition-all duration-500 overflow-hidden group border ${
          ...
          <div className="flex-1 relative w-full h-[760px] sm:h-[640px] md:h-[700px] flex items-center justify-center">
            ...
            <div className={`relative w-full max-w-6xl scale-[1.06] sm:scale-100 transition-all duration-1000 ease-[cubic-bezier(0.23,1,0.32,1)] ${isModelTransitioning ? 'opacity-0 scale-95 blur-2xl translate-y-12' : 'opacity-100 scale-100 blur-0 translate-y-0'}`}>
              ...
              <div className="relative group">
                ...
                <div className="car-frame relative overflow-hidden rounded-[3rem] md:rounded-[4rem] border border-white/20 dark:border-white/10 shadow-[0_40px_100px_rgba(0,0,0,0.3)] dark:shadow-[0_60px_120px_rgba(0,0,0,0.8)]">
                  {prevColor ? (
      ...
      <style>{`
        ...
        .car-layer.is-enter {
          z-index: 2;
          animation: car-luxe-fade 900ms cubic-bezier(0.1, 0.99, 0.16, 1);
        }
        ...
        .car-layer.is-exit {
          ...
          inset: 0;
          z-index: 1;
          animation: car-luxe-fade-out 900ms cubic-bezier(0.1, 0.99, 0.16, 1);
        }
