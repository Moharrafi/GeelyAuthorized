                ...
                <section className="border-t border-slate-200 dark:border-white/10">
                    {(activeCar.id === 'ex5' ? ex5TechnicalSpecs : (activeCar.id === 'ex2' ? ex2TechnicalSpecs : starrayTechnicalSpecs)).map((category) => (
                        <div key={category.title} className="border-b border-slate-200 dark:border-white/10">
                            ...
                            >
                                <h3 className={`text-sm md:text-lg font-black tracking-[0.3em] uppercase transition-colors ${openCategory === category.title ? 'text-accent' : 'text-slate-900 dark:text-white group-hover:text-accent'
                                    }`}>
                                    {category.title}
                                </h3>
                                <div className={`w-8 h-8 rounded-full border flex items-center justify-center transition-all ${openCategory === category.title ? 'border-accent text-accent shadow-[0_0_10px_rgba(56,189,248,0.3)]' : 'border-slate-200 dark:border-white/10 text-slate-400'
                                    }`}>
                            ...
                            <div className={`overflow-hidden transition-all duration-500 ease-in-out ${openCategory === category.title ? 'max-h-[3000px] opacity-100 pb-12' : 'max-h-0 opacity-0'}`}>
                                <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xl">
                                    {/* Sticky Table Header */}
                                    <div className="grid grid-cols-10 md:grid-cols-12 px-4 md:px-10 py-4 md:py-5 bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 transition-all">
                                    ...
                                    {category.specs.map((item, idx) => {
                                        ...
                                        return (
                                            <div key={idx} className={`grid grid-cols-10 md:grid-cols-12 px-4 md:px-10 py-3 md:py-5 border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors items-center ${isDiff ? 'bg-accent/[0.03] dark:bg-accent/[0.05]' : 'bg-white dark:bg-transparent'
                                                }`}>
                                                {/* Label Column */}
